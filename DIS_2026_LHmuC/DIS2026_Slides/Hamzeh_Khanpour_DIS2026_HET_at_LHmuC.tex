...
\title[Perspectives for HET physics studies at \LHCmu]{\justifying {\color{red}Very first draft:} Perspectives for HET physics studies at a muon-hadron collider at CERN}
%\subtitle{DIS2026 -- WG3 Electroweak Physics, Top, Higgs and Beyond the Standard Model}
...
%\vspace{-0.8em}
%\begin{center}
%{\scriptsize Based on recent $\mu$LHC studies and the ongoing HELP4CERN white paper effort}
%\end{center}
...
% Slide 3
% --------------------------------------------------------------------------
\begin{frame}{Talk scope and central message}
\begin{itemize}
%\item This talk focuses on the \textbf{physics case} of the proposed antimuon--hadron collider at CERN, and in particular on the \textbf{Higgs--Electroweak--Top (HET)} programme.
%\item The goal is not to review accelerator or detector R\&D in detail, but to assess what multi-TeV $\mu p$ collisions with high luminosity can deliver for Higgs, top, electroweak, photon-induced, and BSM studies.
\justifying
\item The core message is that \LHCmu{} is \textbf{not only a QCD machine}: it also opens a serious HET factory, with especially strong reach in the \textbf{top sector} and a broad programme in \textbf{precision Higgs, EW, EFT, and $\gamma\gamma$ physics}.
\end{itemize}

\begin{block}{Benchmark working points used throughout the talk}
\small
\justifying 
Baseline: $E_\mu=500$ GeV with $E_p=7$ TeV ($\sqrt{s}\simeq 3.7$ TeV), typically assuming $\mathcal{L}_{\mathrm{int}}=250$ fb$^{-1}$;\\
Upgrade: $E_\mu=1$ TeV with $E_p=7$ TeV ($\sqrt{s}\simeq 5.3$ TeV), typically assuming $\mathcal{L}_{\mathrm{int}}=1000$ fb$^{-1}$.
\end{block}
\small  \scriptsize
{\setlength{\fboxsep}{4pt}\colorbox{blue!20}{\href{https://agenda.infn.it/event/47074/overview}{\strut See talk by Krzysztof Piotrzkowski}}}
\begin{tikzpicture}[remember picture,overlay]
...
% Slide 4
% --------------------------------------------------------------------------
\begin{frame}{Benchmark machines and representative event samples}
\begin{columns} 
\begin{column}{0.99\textwidth}
\begin{itemize}
\justifying
\item Relative to the LHeC, moving from electrons to muons lifts the synchrotron-radiation limitation and pushes the centre-of-mass energy deep into the multi-TeV regime.
%\item For the HET physics case, the useful comparison is between:
%\begin{itemize}
%\justifying
%\item 60 GeV LHeC as the electron--hadron reference,
%\item 500 GeV \LHCmu{} as the baseline HET benchmark,
%\item 1 TeV \LHCmu{} as the upgrade / extended-reach benchmark.
%\end{itemize}
%\item The expected raw event counts are already very large:
%\begin{itemize}
%\justifying
%\item $\sim 1.10\times 10^5$ CC Higgs events at 500 GeV,
%\item $\sim 5.32\times 10^6$ single-top events at 500 GeV,
%\item $\sim 2.23\times 10^6$ $t\bar t$ events at 1 TeV,
%\item $\sim 3.68\times 10^7$ single-top events at 1 TeV.
%\end{itemize}
\end{itemize}
\end{column}
\end{columns}
...
% Slide 5
% --------------------------------------------------------------------------
\begin{frame}{Why a muon-hadron collider? Kinematic reach and physics landscape}
\begin{columns} 
\begin{column}{0.55\textwidth}
\begin{figure}
\centering
\includegraphics[width=\linewidth]{xbjq2_diagram.pdf}
\caption{\justifying \scriptsize Available $(x,Q^2)$ plane for the proposed LHeC and LH$\mu$C, in comparison to existing fixed-target and collider datasets, and the planned EIC kinematic reach. An inelasticity range of $4\times10^{-3}<y<1$ is assumed to evaluate the lower and upper bounds.}
\end{figure}
\end{column}
\begin{column}{0.55\textwidth}
\begin{itemize}
...
\item The same machine that reaches extreme $x$ and $Q^2$ also provides high rates for Higgs, top, EW and photon-induced reactions.
\item \scriptsize
...
\begin{tabular}{lcccc}
\toprule
Process & 20 GeV LHeC & 60 GeV LHeC & 500 GeV \LHCmu{} & 1 TeV \LHCmu{} \\
\midrule
$\sigma(\ell p\to \nu_\ell H+\mathrm{jet}+X)$ [fb] & 52 & 164 & 438 & 731 \\
$\sigma(\ell p\to \ell H+\mathrm{jet}+X)$ [fb] & 5.6 & 19 & 135 & 221 \\
$\sigma(\ell p\to \ell\, t\bar t +X)$ [pb] & 0.0015 & 0.0323 & 1.03 & 2.23 \\
$\sigma(\ell p\to \nu_\ell t +X)$ [pb] & 0.55 & 2.89 & 21.26 & 36.77 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{itemize}
\justifying
\item The qualitative pattern is already clear at the inclusive level: Higgs production rises strongly, while the \textbf{largest gain is in the top sector}.
\item Relative to the 60 GeV LHeC, the 500 GeV \LHCmu{} increases rates by factors of about $\sim2.7$ (CC Higgs), $\sim7$ (single top) and $\sim32$ ($t\bar t$).
\item This table is the cleanest bridge from machine benchmarks to the actual physics case.
\end{itemize}
\begin{tikzpicture}[remember picture,overlay]
\node[anchor=north east, xshift=-0.10cm, yshift=-1.25cm]
at (current page.north east)
...
HELP4CERN collaboration, White Paper on High Energy, High Luminosity Lepton--Hadron Colliders at CERN, in preparation.
}%
}%
}
\end{frame}
...
\begin{itemize}
\justifying
\item Higgs production is dominated by \textbf{charged-current VBF} with a subleading but still sizeable neutral-current contribution.
...
\begin{figure}
\centering
\includegraphics[width=\linewidth]{Higgs.jpg}
\caption{\justifying\scriptsize Higgs boson production cross sections at the LHeC and LH$\mu$C versus centre-of-mass energy, showing the dominant charged-current ($W$-fusion) and subleading neutral-current ($Z$-fusion) contributions.}
\end{figure}
\end{column}
\end{columns}
\begin{tikzpicture}[remember picture,overlay]
...
\item In the HELP4CERN draft, parton showering, fast detector simulation, and a cut-based analysis were used to estimate:
\begin{itemize}
\justifying
...
\item The white-paper draft explicitly highlights:
\begin{itemize}
\justifying
