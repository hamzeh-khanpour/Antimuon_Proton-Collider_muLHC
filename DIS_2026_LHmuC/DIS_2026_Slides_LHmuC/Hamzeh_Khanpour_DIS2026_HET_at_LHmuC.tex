...
%----------------------------------------
\justifying
\includegraphics[width=0.95\linewidth]{Higgs.jpg}
\vspace{0.15em}
{\scriptsize
...
% Slide A
% --------------------------------------------------------------------------
\begin{frame}{Benchmark precision Higgs case: $h\to b\bar b$ in CC VBF}
...
\justifying
\small
\item MadGraph5\_aMC@NLO + Pythia6 + FastJet + Delphes,
...
\item This gives a direct benchmark on the measurable rate
\[
\sigma(p\mu^{+}\to j\,\bar{\nu}_{\mu}\,b\bar b)
\;\propto\;
\frac{g_{hWW}^2\,g_{hbb}^2}{\Gamma_h},
\]
which can be translated into a benchmark sensitivity on $g_{hbb}$.
...
{\scriptsize
\justifying
Benchmark charged-current $h\to b\bar b$ sensitivities in the current HELP4CERN study.
The quoted coupling reach follows from the benchmark rate measurement after propagating
...
% Slide B
% --------------------------------------------------------------------------
\begin{frame}{Broader Higgs-coupling reach: exploratory global-fit picture}
...
\item Beyond the dedicated $h\to b\bar b$ benchmark, the current white-paper draft also
includes an exploratory Higgs coupling fit in the \textbf{kappa-3} framework.
		
\item The compared scenarios are:
\begin{itemize}
\justifying
\small
\item HL-LHC,
\item HL-LHC improved,
\item LHeC,
\item LHeC + LH$\mu$C-500 / 1000 / 2000.
\end{itemize}
				
\item The strongest visible improvement is in the \textbf{vector sector}:
\[
\delta\kappa_W \simeq 0.19\% \;\;(\mathrm{LH}\mu\mathrm{C}\text{-}1000),\qquad
0.16\% \;\;(\mathrm{LH}\mu\mathrm{C}\text{-}2000).
\]
				
\item The companion fermion and loop panels show further, generally milder, gains;
the cleanest conference-level headline is that higher-energy LH$\mu$C stages can make a
particularly strong contribution to \(\kappa_W\) and \(\kappa_Z\).
...
\small
This is a \emph{first explorative} global-fit study.
The draft explicitly assumes LHeC-like systematics and rescales the statistical component
with the benchmark LH$\mu$C cross sections.
Since neither HL-LHC nor any $\ell p$ collider directly measures the Higgs width,
the fit imposes \(\kappa_V\le 1\).
\end{block}
\end{column}
...
The most important message for the talk is the strong projected improvement in the vector sector,
especially for \(\kappa_W\).
}
\end{column}
