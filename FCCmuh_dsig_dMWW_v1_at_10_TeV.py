#!/usr/bin/env python3
"""
FCC_μh_at_10_TeV γγ→W+W− : dσ/dM_WW (SM vs EFT) with CMS-style overlay+ratio and counts/dsig modes.
...
# -----------------------------
def main():
    ap = argparse.ArgumentParser(description="FCC_μh γγ→W+W− : M_WW spectra (SM vs EFT) with CMS-style overlay+ratio.")
    ...
    ap.add_argument("--out-prefix", default="mww_compare_FCC_μh_SM_vs_EFT_at_10_TeV",
                    help="Output prefix for .png/.pdf/.csv")
    ap.add_argument("--title", default=r"FCC_μh@10 TeV: $p(\gamma)\otimes \mu(\gamma)\to W^+W^-$",
                    help="Plot title.")
    ...
    print(f"[FCC_μh] Events with W+W-: SM={Ns}, EFT={Ne}")
    print(f"[FCC_μh] Header σ: SM={meta_sm.get('sigma_pb')} pb ; EFT={meta_eft.get('sigma_pb')} pb")
    ratio = None
    try:
    ...
    if ratio is not None:
        print(f"[FCC_μh] Header σ ratio (EFT/SM) = {ratio:.5f}")
    else:
        print("[FCC_μh] Header σ ratio (EFT/SM) = n/a")
