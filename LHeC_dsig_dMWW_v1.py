#!/usr/bin/env python3
"""
LHeC γγ→W+W− : dσ/dM_WW (SM vs EFT) with CMS-style overlay+ratio and counts/dsig modes.
...
# -----------------------------
def main():
    ap = argparse.ArgumentParser(description="LHeC γγ→W+W− : M_WW spectra (SM vs EFT) with CMS-style overlay+ratio.")
    ...
    ap.add_argument("--title", default=r"LHeC: $p(\gamma)\otimes e(\gamma)\to W^+W^-$",
                    help="Plot title.")
    ...
    print(f"[LHeC] Events with W+W-: SM={Ns}, EFT={Ne}")
    print(f"[LHeC] Header σ: SM={meta_sm.get('sigma_pb')} pb ; EFT={meta_eft.get('sigma_pb')} pb")
    ratio = None
    try:
    ...
    if ratio is not None:
        print(f"[LHeC] Header σ ratio (EFT/SM) = {ratio:.5f}")
    else:
        print("[LHeC] Header σ ratio (EFT/SM) = n/a")
