...
%   Hamzeh Khanpour
%   Ultra-cold muons for the LHC, 24–26 September 2026 at AGH University of Kraków
%   Two-photon Physics
...
\title[Two-photon Physics at \LHCmu]{\justifying Two-photon Physics at a lepton-hadron collider at CERN}
...
\date{{\setlength{\fboxsep}{4pt}\colorbox{cyan!15}{\strut AGH University of Kraków, 24-26 September, 2026}}}
...
\justifying
\small
\item The present talk focuses on the physics case of the LH$\mu$C, using the current working benchmarks: 
...
\justifying
\scriptsize
These working points are sufficient to demonstrate the main message of LH$\mu$C: already at the 500~GeV, LH$\mu$C enters the multi-TeV regime and becomes highly competitive for Higgs, top, electroweak, $\gamma\gamma$, and representative BSM studies.
\end{block}
}
\centering
\tiny
...
\colorbox{green!20}{\href{https://agenda.infn.it/event/47074/contributions/289190/}{\strut See talk by Krzysztof Piotrzkowski}}%
}
\end{column}
\end{columns}
\end{frame}
...
\centering
\scriptsize
{\setlength{\fboxsep}{4pt}\colorbox{green!20}{\href{https://agenda.infn.it/event/47074/contributions/289240/}{\strut See talk by Piotr Kotko}}}
...
even the 500~GeV option opens a qualitatively new regime for electroweak, EFT, and BSM physics.
\end{block}
\centering
\tiny
{\setlength{\fboxsep}{4pt}\colorbox{yellow!40}{\strut From HELP4CERN White Paper (in preparation)}}
...
\item A concrete benchmark study already exists for the CC VBF channel~~ {\color{magenta}$
p\mu^{+}\to j\,\bar{\nu}_{\mu}\,h; \,\, h\to b\bar b$}.
...
\item The statistical uncertainty estimated as $\frac{\Delta\sigma}{\sigma}
...
\[ \sigma(p\mu^{+}\to j\,\bar{\nu}_{\mu}\,b\bar b) \;\propto\; \frac{g_{hWW}^2\,g_{hbb}^2}{\Gamma_h}\,. \]
...
\vspace{0.15em}			
{\scriptsize
Benchmark CC \(h\to b\bar b\) sensitivities.
The coupling reach is obtained from the rate measurement after propagating
the external uncertainty on \(g_{hWW}^2/\Gamma_h\).
}
%\vspace{0.2em}
...
\(g_{hbb}\) in the CC Higgs channel, improving on the LHeC expectation of about
{\color{blue}\(0.97\%\)}
[\href{https://indico.cern.ch/event/568360/contributions/2523555/attachments/1440097/2216668/mtanaka_DIS2017_SMhiggs_final.pdf}
{{\color{magenta}M.~Tanaka, DIS2017}}]
and substantially exceeding the HL-LHC expectation of about {\color{blue}\(4\%\)}
[{\color{magenta}1902.00134 [hep-ph]}].
\end{alertblock}
...
examined a Higgs coupling fit in the kappa-3 framework {\scriptsize [{\color{magenta}JHEP 01 (2020) 139}]}.
...
\item and the fit results shown here were obtained with the {\color{blue}\textsc{SMEFiT} framework} [{\color{magenta}2604.16596}].
\end{itemize}
...
{\scriptsize
\setlength{\fboxsep}{4pt}%
\colorbox{green!20}{\href{https://agenda.infn.it/event/47074/contributions/289202/}{\strut Higgs\&BSM@LHeC: Néstor Armesto}}%
}
...
% --------------------------------------------------------------------------
% New Slide: fermion and loop sectors
% --------------------------------------------------------------------------
\begin{frame}{Broader Higgs-coupling reach: fermion and loop sectors}
...
\begin{column}{0.50\textwidth}
\centering
\includegraphics[width=\linewidth]{LHmuC_fermion.pdf}
\end{column}
		
\begin{column}{0.50\textwidth}
\centering
\includegraphics[width=\linewidth]{LHmuC_loop.pdf}
\end{column}
...
%----------------------------------------
% Bottom row: concise explanation
%----------------------------------------
\begin{itemize}
\justifying
\small
...
\item  Fermion sector: 
the LH$\mu$C can substantially improve the
precision on selected fermion couplings, especially
\(\kappa_b\), \(\kappa_c\), and \(\kappa_\tau\).
Already at the \(1\)~TeV stage, the combined LHeC+LH$\mu$C fit becomes
clearly stronger than the HL-LHC benchmark for \(\kappa_b\) and \(\kappa_c\).   

\item  Loop sector: 
loop-induced modifiers are not the primary strength of a VBF-driven Higgs programme.
Nevertheless, the \(1\)~TeV and \(2\)~TeV stages make
\(\kappa_g\) competitive with, and eventually better than, the HL-LHC benchmark.    
...
{\scriptsize
\setlength{\fboxsep}{4pt}%
\colorbox{green!20}{\href{https://agenda.infn.it/event/47074/contributions/289190/}{\strut HET@LHeC: Paul Newman}}%
}
\end{itemize}
\end{alertblock}
...
\item In the nominal \(5.3~\mathrm{TeV}\) baseline, the effective $\gamma\gamma$ centre-of-mass energy extends into the multi-TeV regime, opening a broad programme of clean exclusive measurements.
\justifying
\footnotesize
\setlength{\itemsep}{0.1em}
\begin{itemize}
\item anomalous $\tau$ moments,
\item $\gamma\gamma\to W^+W^-$ and anomalous quartic gauge couplings (aQGC),
...
{\scriptsize
\setlength{\fboxsep}{4pt}%
\colorbox{green!20}{\href{https://agenda.infn.it/event/47074/contributions/289063/}{\strut $\gamma \gamma$@UPC@ATLAS:  Lydia Beresford
}}%
}
...
% --------------------------------------------------------------------------
\begin{frame}{Top physics at \LHCmu{}: production mechanisms}
...
\item The top sector provides one of the clearest HET benchmarks for \LHCmu{}, since both single-top and \(t\bar t\) production become statistically powerful channels at the nominal baseline.

\item {\color{magenta}Single-top production} is dominated by charged-current DIS, with representative subprocess $\mu^{+} b \to \bar{\nu}_{\mu}\, t ,$ 
mediated by \(t\)-channel \(W^{+}\) exchange in the 5-flavor scheme.

\item {\color{magenta}Top-pair production} is dominated by neutral-current lepton--gluon scattering, with representative process $\mu^{+} g \to \mu^{+} t\bar t ,$
...
% Slide: RPV squarks
% --------------------------------------------------------------------------
\begin{frame}{RPV squarks at \texorpdfstring{\LHCmu{}}{LHmuC}: resonant \(\mu q\) production}
...
\scriptsize
\justifying
\item R-parity-violating SUSY allows lepton-quark-squark couplings.  The relevant superpotential terms are
				\[
				...
				\]
\item The LH\(\mu\)C benchmark focuses on the \(LQD\)-type term
				\[
				W_{\rm RPV}\supset \lambda'_{ijk}L_iQ_j\bar D_k ,
				\]
where \(i=2\) selects the muon generation.
\end{itemize}
...
\scriptsize
\justifying
\item The neutral-current benchmark is $p\mu^+ \to d\,\mu^+$, mediated by a resonant squark in the \(\mu q\) channel.
				
\item The signal is selected with one reconstructed muon and at least one jet; high jet-\(p_T\)-sum thresholds suppress the DIS-like SM background.
				
\item In favourable neutral-current channels, \(\lambda'_{2j1}\sim0.01\text{-}0.02\)
can be tested for \(m_{\tilde q}\sim1\text{-}3~\mathrm{TeV}\).
				
[{\centering
\tiny
{\setlength{\fboxsep}{4pt}\colorbox{magenta!10}{\strut Phys. Rev. D 103 (2021) 116009.}}}]
\end{itemize}
...
\includegraphics[width=0.95\linewidth]{RPV_1.jpg}
			
			%\vspace{-0.5em}
			%{\tiny
				%NC \(p\mu^+\to d\mu^+\), \(E_\mu=500~\mathrm{GeV}\), %\(250~\mathrm{fb}^{-1}\).
				%}
			
\vspace{0.1em}
			
\includegraphics[width=0.95\linewidth]{RPV_2.jpg}
			
			%\vspace{-0.5em}
			%{\tiny
				%NC \(p\mu^+\to d\mu^+\), \(E_\mu=1~\mathrm{TeV}\), %\(500~\mathrm{fb}^{-1}\).
				%}
			
			%\vspace{0.4em}
			
			%\begin{exampleblock}{Reading the curves}
			%\tiny
			%\justifying
			%The curves show 95\% CL limits from \(S=N_s/\sqrt{N_b}=2\).
			%Lower \(\lambda'\) means stronger reach; red curves show LHC/HL-LHC reference bounds.  Higher jet-\(p_T\)-sum cuts improve the heavy-squark reach.
			%\end{exampleblock}
...
% --------------------------------------------------------------------------
% Slide: color-octet muons / leptogluons
% --------------------------------------------------------------------------
\begin{frame}{Color-octet muons at \texorpdfstring{\LHCmu{}}{LHmuC}: resonant \(\mu g\) production}
...
\scriptsize
\justifying
\item In composite-lepton scenarios, the SM muon can have a color-octet partner
\(\mu_8\), also called a leptogluon.  The effective interaction is 
[{\centering
\tiny
{\setlength{\fboxsep}{4pt}\colorbox{magenta!10}{\strut2506.15445 [physics.acc-ph]}}}]
				\[
				\mathcal{L}
				=
				\frac{1}{2\Lambda}
				...
				g_s G^{\alpha}_{\mu\nu}\,
				\sigma^{\mu\nu}
				\left(\eta_L \ell_L+\eta_R \ell_R\right)
				+{\rm h.c.}
				\right\}.
				\]
Here \(\ell^\alpha_8\) is the color-octet lepton, \(G^\alpha_{\mu\nu}\) is the gluon field strength, \(\Lambda\) is the compositeness scale, and
\(\eta_{L,R}\) are chiral factors.
\end{itemize}
...
\scriptsize
\justifying
\item The benchmark assumes \(\Lambda=m_{\mu_8}\) and targets the resonant
single-production channel  $\mu g\to\mu_8\to\mu g$.
				
\item Signal topology: $\mu p\to \mu_8 X\to \mu j X$, with one high-\(p_T\) muon and one high-\(p_T\) jet.
				
\item Dominant background: $\mu p\to \mu j X$, from SM neutral-current DIS-like scattering.
	
\item The key advantage is the direct \(\mu g\) initial state; \(pp\) searches mainly rely on QCD pair production.
\end{itemize}
...
\includegraphics[width=0.95\linewidth]{mu8.jpg}
			
\vspace{0.25em}
			
\begin{exampleblock}{Representative \(5\sigma\) reach}
\scriptsize
\centering
\begin{tabular}{lc}
\hline
Setup & \(m_{\mu_8}\) reach \\
\hline
HL-LHC, \(1~{\rm ab}^{-1}\) & \(\sim 2.3~{\rm TeV}\) \\
\LHCmu{}, \(E_\mu=500~{\rm GeV}\), \(250~{\rm fb}^{-1}\) & \(\sim 3.0~{\rm TeV}\) \\
\LHCmu{}, \(E_\mu=1~{\rm TeV}\), \(500~{\rm fb}^{-1}\) & \(\sim 4.2~{\rm TeV}\) \\
\hline
\end{tabular}
\end{exampleblock}
...
\begin{itemize}
\justifying
\item The long-term perspective goes well beyond the 5.3 TeV baseline.
...
\item In particular, a future FCC-$\mu h$ machine, with representative 
\(\sqrt{s_{\mu p}}\sim 15\) and \(30~\mathrm{TeV}\) configurations would extend:
\begin{itemize}
\justifying
...
% --------------------------------------------------------------------------
% Extra slide: current a_tau and d_tau context
% --------------------------------------------------------------------------
\begin{frame}{Current constraints on \(a_\tau,d_\tau\)}
...
\vspace{-0.2em}
\centering
\includegraphics[width=0.8\linewidth]{y_tau_tau.jpg}
			
\vspace{0.4em}
			
\begin{block}{Benchmark topology}
\scriptsize
\justifying
Photon-induced
				\[
				\gamma\gamma\to\tau^+\tau^-
				\]
directly probes the \(\gamma\tau\tau\) vertex.  It is therefore a clean
	channel for anomalous magnetic and electric dipole moments,
	\(a_\tau\) and \(d_\tau\).
\end{block}
...
\begin{block}{Current 95\% CL constraints on \(a_\tau\)}
\tiny
\centering
\begin{tabular}{l l c c}
\hline
Experiment & Channel & Dataset & \(a_\tau\) interval \\
\hline
DELPHI & \(e^+e^-\to e^+e^-\tau^+\tau^-\) &
					\(650~{\rm pb}^{-1}\) &
					\([-0.052,\,0.013]\) \\
...
\textcolor{magenta}{CMS 2024} &
\textcolor{magenta}{\(pp\) exclusive \(\gamma\gamma\to\tau\tau\)} &
\textcolor{magenta}{\(138~{\rm fb}^{-1}\)} &
\textcolor{magenta}{\([-0.0022,\,0.0041]\)} \\
					
\textcolor{magenta}{ATLAS 2025} &
\textcolor{magenta}{\(pp\) high-mass \(\tau\tau\)} &
\textcolor{magenta}{\(140~{\rm fb}^{-1}\)} &
\textcolor{magenta}{\([-0.0024,\,0.0047]\)} \\
\hline
\end{tabular}
\end{block}
...
\scriptsize
\justifying
\item The SM expectation is
				\[
				a_\tau^{\rm SM}\simeq 117721\times10^{-8}\simeq1.18\times10^{-3}.
				\]
				
\item CMS 2024 also reports
				\[
				a_\tau=0.0009^{+0.0032}_{-0.0031},
				\qquad
				|d_\tau|<2.9\times10^{-17}\,e\,{\rm cm}
				\quad (95\%~{\rm CL}).
				\]
				
\item Present LHC constraints are therefore already at the few-\(10^{-3}\)
level for \(a_\tau\).  The LH\(\mu\)C motivation is a much larger
high-mass exclusive \(\tau^+\tau^-\) sample.
\end{itemize}
...
% --------------------------------------------------------------------------
% Backup slide: technical concept behind LHmuC
% --------------------------------------------------------------------------
\begin{frame}{Technical concept behind the LH$\mu$C proposal}
	
\begin{columns}[T]%[T,onlytextwidth]
		
%========================================
% Left column: layout figure
%========================================
\begin{column}{0.5\textwidth}
\centering
\includegraphics[width=\linewidth]{LHmuC_ring.jpg}
			
\vspace{0.3em}
\scriptsize
\centering
Conceptual layout of the proposed LH$\mu$C configuration.
\end{column}
		
%========================================
% Right column: technical message
%========================================
\begin{column}{0.55\textwidth}
\small
...
\item The LH$\mu$C proposal aims to re-use key elements of the
LHeC infrastructure to make high-energy, high-luminosity
\(\mu^+ p\) collisions. 
				
\item The basic ingredients are:
\begin{itemize}
\small
\justifying
\item an ultra-cold positive-muon source,
\item acceleration / booster stages for the \(\mu^+\) beam,
\item a dedicated \(\mu^+\) storage ring. 
\end{itemize}
{\scriptsize
\setlength{\fboxsep}{4pt}%
\colorbox{blue!20}{\href{https://agenda.infn.it/event/47074/contributions/289190/}{\strut See talk by Krzysztof Piotrzkowski}}%
}				
\end{itemize}
...
(1) \(\mu\)TRISTAN: Y.~Hamada \textit{et al.},
\emph{Prog.\ Theor.\ Exp.\ Phys.} \textbf{2022} (2022) 053B02. 
	
\vspace{0.25em}

(2) \(\mu\)LHC: Antimuon Ring and High Luminosity-LHC-Based \(\mu^+p\) Collider: D.~Akturk \textit{et al.},
\emph{Prog.\ Theor.\ Exp.\ Phys.} \textbf{2026} (2026) 033C04.

\vspace{0.25em}

(3) HELP4CERN White Paper (in preparation)
	...
	\begin{columns}[T]%[T,onlytextwidth]
		...
		\begin{column}{0.60\textwidth}
			...
			\begin{itemize}\itemsep0.42em
				...
				In favourable NC [$p\mu^+ \to d\,\mu^+$] benchmark channels, 
				...
				\mu g \to \mu_8 \to \mu g ,
				\]
				whereas \(pp\) searches mainly rely on pair production [{\centering
					\tiny
					{\setlength{\fboxsep}{4pt}\colorbox{magenta!10}{\strut2506.15445 [physics.acc-ph]}}}].
