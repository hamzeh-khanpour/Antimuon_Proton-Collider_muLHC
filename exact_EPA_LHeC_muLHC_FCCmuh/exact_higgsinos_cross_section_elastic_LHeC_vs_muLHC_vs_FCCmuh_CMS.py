#!/usr/bin/env python3
# Compare integrated σ(γγ → \tilde{H}^+\tilde{H}^-) ; W > W0) using ELASTIC photon–photon luminosities
# LHeC (e–p, Ee=50 GeV, Ep=7 TeV) vs μLHC (μ–p, Eμ=500 GeV, Ep=7 TeV) vs FCCmuh (μ–p, Eμ=500 GeV, Ep=50 TeV)
...
#
# Outputs:
#   - exact_higgsinos_cross_section_elastic_LHeC_vs_muLHC_FCCmuh.pdf/.png
#   - exact_higgsinos_cross_section_elastic_LHeC_vs_muLHC_FCCmuh.txt  (W0, sigma_LHeC[pb], sigma_muLHC[pb], sigma_FCCmuh[pb])
...
W_LHeC,   S_LHeC   = load_sgg(f_LHeC)
W_muLHC,  S_muLHC  = load_sgg(f_muLHC)
W_FCCmuh, S_FCCmuh = load_sgg(f_FCCmuh)

# ---------------- γγ → ττ Born σ(W) (pb) ----------------
# Same formulaic structure as in your file, but with the correct threshold W >= 2 m_tau.
def cs_tautau_W(W):
    alpha = 1.0/137.0
    hbarc2 = 0.389  # (GeV^2·mb) → we end in pb via 1e9 factor below
    m_tau = 200.0 # 1.77686 (kept as-is: placeholder heavy charged mass)
...
W0_e,  Sig_e  = integrate_sigma(W_LHeC,   S_LHeC)
W0_mu, Sig_mu = integrate_sigma(W_muLHC,  S_muLHC)
W0_FCC, Sig_FCC = integrate_sigma(W_FCCmuh, S_FCCmuh)
...
# ---------------- Save the numbers ----------------
out_txt = "exact_higgsinos_cross_section_elastic_LHeC_vs_muLHC_FCCmuh.txt"
np.savetxt(out_txt,
...
ax.set_ylabel(r"$\sigma(\gamma\gamma\to\tilde{H}^+\tilde{H}^-;\, W>W_0)$  [pb]")
...
plt.savefig("exact_higgsinos_cross_section_elastic_LHeC_vs_muLHC_FCCmuh.pdf")
plt.savefig("exact_higgsinos_cross_section_elastic_LHeC_vs_muLHC_FCCmuh.png", dpi=300)
plt.show()
