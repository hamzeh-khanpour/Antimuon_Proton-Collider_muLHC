...
#  This code also calculates the integrated "inelastic" tau tau cross section (mu p -> mu p tau^+ tau^-)
#  Hamzeh Khanpour, Laurent Forthomme, and Krzysztof Piotrzkowski -- November 2024
#  Updated in November 2025 to account for the muLHC
################################################################################
...
mumass = 0.1056583755   # muon mass in GeV
pmass = 0.938272081    # Proton mass in GeV
pi0mass = 0.1349768    # Pion mass in GeV

q2emax = 10000000.0  # Maximum photon virtuality for electron in GeV^2
q2pmax = 10000000.0  # Maximum photon virtuality for proton in GeV^2
...
def flux_y_electron(ye, lnQ2e):
    ...
    if ye <= 0 or ye >= 1:
        return 0.0
    qmin2v = qmin2_electron(mumass, ye)
...
def flux_el_yy_atW(W, eEbeam, pEbeam):
    ...
    def integrand(ye):
        ...
        qmin2e = qmin2_electron(mumass, ye)
        if qmin2e <= 0:
            return 0.0
        ...
        def lnQ2e_integrand(lnQ2e):
            ...
            MN_max = 300.0  # Given as a fixed upper bound for MN
...
# Parameters
eEbeam = 600.0  # Electron beam energy in GeV
pEbeam = 7000.0  # Proton beam energy in GeV
W_values = np.logspace(1.0, 3.0, 303)  # Range of W values from 10 GeV to 1000 GeV

num_cores = 50  # Set this to the number of cores you want to use
...
# Save results with None handling and formatted filename
    filename = f"muLHC_Inelastic_Photon_Luminosity_Spectrum_MNmax_{int(MN_max)}_q2emax_{int(q2emax)}_q2pmax_{int(q2pmax)}.txt"
...
# Construct filenames for the plots including MN_max, q2emax, and q2pmax
    plot_filename_pdf = f"muLHC_Inelastic_Photon_Luminosity_Spectrum_MNmax_{int(MN_max)}_q2emax_{int(q2emax)}_q2pmax_{int(q2pmax)}.pdf"
    plot_filename_jpg = f"muLHC_Inelastic_Photon_Luminosity_Spectrum_MNmax_{int(MN_max)}_q2emax_{int(q2emax)}_q2pmax_{int(q2pmax)}.jpg"
