#!/usr/bin/env python3
"""
muLHC _at_7-5_TeV one-lepton + two-jet analysis with 'overflow -> last bin' folding.
...
def save_plot_and_csv(x, y_sm, y_eft, edges, out_prefix: str, xlabel: str,
                      ...
                      mode: str="counts", lumi_fb: float=100.0,
...
def analyze_one_sample(path: str,
    ...
    for w_ev, ev in parse_lhe_events_with_weights(path):
        ...
        # Optional event-level pT(ℓ) gate
        if pt_lep_min and pt_lep_min > 0.0:
            if lep is None or pT(lep["px"], lep["py"]) <= pt_lep_min:
                continue
        if lep is not None:
...
def main():
    ap = argparse.ArgumentParser(description="muLHC (γγ→WW→ℓ jj) spectra with overflow folding.")
    ...
    ap.add_argument("--pt-lepton", type=float, default=0.0,
                    help="Event-level gate: keep event only if hardest charged lepton pT > value (GeV). 0 disables.")
    ...
    # plot + CSV (counts by default @ muLHC lumi)
    mode = args.yield_mode
    Lfb  = args.lumi_fb
    cut_tag = f"__ptlgt{int(args.pt_lepton)}" if (args.pt_lepton and args.pt_lepton>0) else ""

    save_plot_and_csv(c_lep, y_lep_sm, y_lep_eft, e_lep,
                      f"pt_lepton_muLHC_SM_vs_EFT_at_7-5_TeV{cut_tag}",
                      r"$p_T(\ell)$ [GeV]",
    ...
    save_plot_and_csv(c_j1, y_j1_sm, y_j1_eft, e_j1,
                      f"pt_j1_muLHC_SM_vs_EFT_at_7-5_TeV{cut_tag}",
                      r"$p_T(j_1)$ [GeV]",
                      args.logy, label(sm["meta"], "SM"), label(eft["meta"], "EFT"),
                      sm_ls="--", eft_ls="-", mode=mode, lumi_fb=Lfb,
    ...
    save_plot_and_csv(c_j2, y_j2_sm, y_j2_eft, e_j2,
                      f"pt_j2_muLHC_SM_vs_EFT_at_7-5_TeV{cut_tag}",
                      r"$p_T(j_2)$ [GeV]",
                      args.logy, label(sm["meta"], "SM"), label(eft["meta"], "EFT"),
                      sm_ls="--", eft_ls="-", mode=mode, lumi_fb=Lfb,
    ...
    save_ratio_plot_csv(c_lep, e_lep, y_lep_sm, y_lep_eft, err_lep_sm, err_lep_eft,
                        f"pt_lepton_muLHC_SM_vs_EFT_ratio_at_7-5_TeV{cut_tag}", r"$p_T(\ell)$ [GeV]", mode, Lfb)
    save_ratio_plot_csv(c_j1,  e_j1,  y_j1_sm,  y_j1_eft,  err_j1_sm,  err_j1_eft,
                        f"pt_j1_muLHC_SM_vs_EFT_ratio_at_7-5_TeV{cut_tag}",     r"$p_T(j_1)$ [GeV]", mode, Lfb)
    save_ratio_plot_csv(c_j2,  e_j2,  y_j2_sm,  y_j2_eft,  err_j2_sm,  err_j2_eft,
                        f"pt_j2_muLHC_SM_vs_EFT_ratio_at_7-5_TeV{cut_tag}",     r"$p_T(j_2)$ [GeV]", mode, Lfb)
