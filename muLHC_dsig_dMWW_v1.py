...
- Original μLHC script structure & LHE parsing.  [file cite]  # :contentReference[oaicite:15]{index=15}
- CMS-style helpers (mplhep style, panel text, label pattern).  [file cite]  # :contentReference[oaicite:16]{index=16}
- Target look for M_WW plot & legend content.  [file cite]  # :contentReference[oaicite:17]{index=17}
"""
...
# Try to use mplhep "CMS" style; fall back silently if not installed.  [from CMS script]  # :contentReference[oaicite:18]{index=18}
try:
...
# -----------------------------
# LHE header parsing (MG5-like)  [from μLHC script]  # :contentReference[oaicite:19]{index=19}
# -----------------------------
...
# -----------------------------
# Events + weights  [from μLHC script]  # :contentReference[oaicite:20]{index=20}
# -----------------------------
...
# -----------------------------
# Physics helpers  [from μLHC script]  # :contentReference[oaicite:21]{index=21}
# -----------------------------
...
# -------- step with last-plateau (CMS look)  [from CMS script/ref]  # :contentReference[oaicite:22]{index=22} :contentReference[oaicite:23]{index=23}
def step_with_plateau(edges, y):
    x = np.r_[edges[:-1], edges[-1]]
    yy = np.r_[y, y[-1] if len(y) else 0.0]
    return x, yy

# -------- small annotation panel (trimmed CMS helper)  [ported]  # :contentReference[oaicite:24]{index=24}
...
def plot_overlay_and_ratio(edges, y_sm, dy_sm, y_eft, dy_eft,
    ...
    # small info panel
    # If no fiducial selection, equal gen=fid to keep panel consistent with CMS look
    ...
    # bottom: ratio (invariant under mode since it's bin-by-bin, same Δ and lumi)
    ...
    # draw a light band for stat. ratio uncertainty
...
def main():
    ...
    # Gather M_WW and weights (W+W- four-vectors as in the original μLHC script)  # :contentReference[oaicite:25]{index=25}
    def loop_file(path, w_fallback):
        Ms, wts = [], []
    ...
    Ms_sm,  w_sm  = loop_file(args.sm_lhe,  w_fallback_sm)
    Ms_eft, w_eft = loop_file(args.eft_lhe, w_fallback_eft)
    ...
    # Labels incl. header σ and N (CMS style)  # :contentReference[oaicite:26]{index=26}
