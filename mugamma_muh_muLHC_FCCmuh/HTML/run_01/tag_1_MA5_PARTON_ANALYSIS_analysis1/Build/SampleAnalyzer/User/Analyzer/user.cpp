...
MAbool user::Execute(SampleFormat& sample, const EventFormat& event)
{
  ...
  // Histogram number 9
  //   * Plot: DELTAR ( h[1] , mu+[1] ) 
  {
  {
    MAuint32 a[1];
    for (a[0]=0;a[0]<_P_h_I1I_PTorderingfinalstate_REG_.size();a[0]++)
    {
    MAuint32 b[1];
    for (b[0]=0;b[0]<_P_mu_p_I1I_PTorderingfinalstate_REG_.size();b[0]++)
    {
      Manager()->FillHisto("9_DELTAR", _P_h_I1I_PTorderingfinalstate_REG_[a[0]]->dr(_P_mu_p_I1I_PTorderingfinalstate_REG_[b[0]]));
    }
    }
  }
  }
